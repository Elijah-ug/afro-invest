...
import { useSendRequestMutation } from '@/store/features/withdrawReq';
  ...
  const handlePurchasePlan = async () => {
    ...
    try {
      if (isDeposit) {
        ...
        const res = await request({ amount: parseFloat(userPlan.amount) } as any);
        if (res.data) {
          return toast.success(res.data.message);
        }
        console.log('Withdraw request:', res);
      }
    } catch (error) {
    ...
    <Card className='shadow-md border-muted/40'>
      ...
      <CardContent className='space-y-5'>
        ...
        <div className='space-y-2'>
          <Label htmlFor={type}>Amount (USDC)</Label>
          <Input
            id={type}
            type='number'
            placeholder='0.00'
        ...
        {isDeposit && (
          <div className='space-y-2 '>
            <Label htmlFor={type}>Select Duration (Days)</Label>
        ...
        >
          {isPending || isLoading ? <Spinner className='size-8' /> : isDeposit ? '💰 Deposit Now' : '💸 Withdraw Now'}
        </Button>
      </CardContent>
    </Card>
  );
